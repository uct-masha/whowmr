...
The World Health Organization (WHO) publishes a World Malaria Report annually. Since 2017, the publication additionally includes annexes in Excel format containing detailed data on various aspects of malaria epidemiology and control efforts. The `whowmr` package provides a user-friendly interface to access and analyze this data in R. The package includes processed versions of the annexes from the World Malaria Reports starting from 2017 and a function to download the PDF reports themselves.
...
To access specific data from the annexes, use the appropriate dataset object and sheet name. For instance, `whowmr::wmr2021$wmr2021c` would provide the funding information from the 2021 report's annex 4C.

The package also includes a function `download_whowmr_reports(years=2017:2025, outdir='.')` to download PDF copies of the World Malaria Reports for the specified years. The reports are downloaded and saved in the specified output directory.
